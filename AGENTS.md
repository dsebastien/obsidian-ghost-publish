...
- The preset list keeps its custom row UI (enable toggle, move, edit-in-modal,
  ...
  serialized behind a latch and hand `updateSettings` mutations that derive
  from the committed draft; the pane re-renders only after the write lands.
- `excludedFolders`, `stripSections` and `knownUrls` are textarea controls
  that serialize in `getControlValue` and parse in `setControlValue`.
- `cachedTags` / `cachedNewsletters` / the fetch timestamps are plugin-owned
  cache data sharing data.json with the settings — never surface them as
  controls; the serialized write path is what keeps a settings edit from
  clobbering a concurrent cache refresh.
